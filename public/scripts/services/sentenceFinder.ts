module Speaker {

  export interface SentenceFinderInterface {
    findSentence(word: string);
  }

  class SentenceFinderService implements SentenceFinderInterface {

    // fields
    static $inject = ["$rootScope"];
    $rootScope: ng.IScope;

    findSentence(word: string, token: string) {
      //https://www.wordsapi.com/
      $.ajax({
				type: "POST",
				url: 'https://wordsapiv1.p.mashape.com/words/' + word + '?accessToken='+token,
				contentType: "application/json; charset=utf-8",
				dataType: "json",
				success: function (data) {
					var requestId = data.d;
					$('#blog_subscription_modal #btnSubmitBlogUpdates')
						.attr("data-request-id", requestId)
						.attr("data-blog", blog)
						.attr("data-course", course)
						.trigger("subscriptionAdded");

					if (isValid) {
						localStorage.setItem('blogPopStateNew', 'shown');
					}
				},
				error: function (data) {
				}
			});
    }

    // methods
    constructor($rootScope: ng.IScope) {
      this.$rootScope = $rootScope;
    }
  }

  angular
    .module("app")
    .service("SentenceFinderService", SentenceFinderService);
}
